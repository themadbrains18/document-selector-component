...
const EligibleForBenefits = () => {
  ...
  const [customPeriod, setCustomPeriod] = useState(90); // State for the custom period
        ...
        <RadioGroup.Option value='yes'>
          {({ checked }) => (
            <div className='flex gap-2 items-center cursor-pointer'>
              <span
                className={`inline-block w-4 h-4 rounded-full  bg-gray-50 border-[0.5px] border-gray-300 ${
                  checked ? 'border-[3.5px] border-orange-500' : ''
                }`}
              ></span>
                ...
                <div className='flex items-center gap-4'>
                  ...
                  <div className='flex gap-2 items-center'>
                    <div className='px-4 py-2 rounded-lg border border-gray-300 bg-white'>
                      <input
                        type='number'
                        className='bg-transparent w-[42px] h-5 block outline-none'
                        placeholder='90'
                        value={checked ? customPeriod : ''}
                    ...
                    <Menu as='div' className='min-w-40 w-full relative bg-white'>
                      <Menu.Button
                        className={`border border-gray-300 rounded-lg px-2.5 py-2 flex justify-between gap-2.5 items-center cursor-pointer min-w-40 w-full`}
                        onClick={() => setToggle(!toggle)}
                        disabled={!checked}
                      >
                        <span className={`font-inter text-sm  ${!checked ? 'text-gray-400' : 'text-gray-900'}`}>
                          {selected ? selected : ''}
                        </span>
                        <img
                          src='/icons/arrow-down.svg'
                          alt='Arrow down icon'
                          className={`w-2.5 h-2.5 ${!checked && 'opacity-50'}`}
                        />
                      </Menu.Button>
                      <ul
                        className={`border border-gray-200 rounded-lg shadow-md p-1.5 mt-0.5 duration-300 absolute bg-white w-full ${
